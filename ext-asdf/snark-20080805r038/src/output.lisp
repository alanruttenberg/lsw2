...
(defun print-term3 (term &optional (stream *standard-output*) depth)
  ;; compound-appl, variable, function-symbol struct printer
  (declare (ignore depth))
  (print-term term *subst-for-printing* stream))

(defun print-term (term &optional subst (stream *standard-output*))
  ;; terms are printed by first converting them to lisp
  ;; by replacing compound structures by conses, function structures by symbols, etc.
  (let ((x (let-options ((variable-to-lisp-code nil)) (term-to-lisp term subst))))
    (with-standard-io-syntax2
      (cond
       ((variable-p x)
        ;; variables may not be changed by term-to-lisp, print them here
       ...
       ((and (consp x)					;override printing of 'a and #'a for (quote a) and (function a)
             (or (eq 'quote (first x)) (eq 'function (first x)))
             (rest x)
             (null (rrest x)))
        (princ #\( stream)
        (prin1 (first x) stream)
        (princ #\  stream)
        (prin1 (second x) stream)
        (princ #\) stream))
       (t
        (prin1 x stream))))
    term))
...
(defun print-row (row &key (string "Row ") format)
  (when row
    (ecase format
      ((nil)
       (with-standard-io-syntax2
  ...
  (labels
	...
	:if-constant (let ((name (constant-name term)))
                       (cond
                        ...
                        (t
                         name)))
	:if-variable (dolist (fun (mklist (variable-to-lisp-code?)) term)
	...
	:if-compound (let ((head (head term))
                       ...
                       (cond
                        ...
                        (t
                         (dolist (fun (function-to-lisp-code head) (cons (function-name head) (args-to-lisp args)))
...
(defun cons-term-to-lisp (head args subst)
  ...
  (let* ((y (term-to-lisp (second args) subst))
    ...
    (cond
     ((null y)
      (list (function-name (find-symbol-table-entry '$$list :function :any)) x))
     ((atom y)
      (list (function-name head) x y))
     (t
      (let ((v (first y)) list*)
        (cond
         ((eq v (function-name (find-symbol-table-entry '$$list :function :any)))
          (list* v x (rest y)))
         ((or (eq v (setf list* (function-name (find-symbol-table-entry '$$list* :function :any))))
