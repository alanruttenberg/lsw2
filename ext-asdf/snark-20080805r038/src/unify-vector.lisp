...
(defun unify-identity-with-vector (cc terms subst fn identity)
  (let ((vars nil) term)
    (loop
      (multiple-value-setq (term terms) (first-and-rest-of-vector terms subst fn identity))
      (cond
	((eq none term)
         (dolist (var vars)
...
(defun unify-variable-with-vector (cc var arg args subst fn identity max)
  ...
  (when (and (implies max (<= 2 max))
             (subsort? (associative-function-sort fn) (variable-sort var)))
    (let ((l nil)
          (count 0))
      (loop
        (cond
         ...
         (t
          ...
          (multiple-value-setq (arg args) (first-and-rest-of-vector args subst fn identity))))))))
