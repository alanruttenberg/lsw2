  ...
  (let ((top-uri (if (stringp ontology) ontology (v3kb-name ontology)))
    ...
    (let ((imported->import (make-hash-table :test 'equal)))
	 ...
	 for partial-path = (if (equal ontology-iri top-uri)
				...
				(#"replaceAll" ontology-iri "^.*//" ""))
	 for fetch-cmd = (format nil "~a --quiet --output-document \"~a~a\" ~s"  wget-command (namestring directory) partial-path source)
	 do (princ fetch-cmd)
	...
	(loop for (imported-iri ontology-iri) in (loaded-documents ontology)
	      with seen = (make-hash-table :test 'equalp)
	   for partial-path = (if (equal ontology-iri top-uri)
				  (#"replaceAll" ontology-iri "^.*[#/]" "")
				  (#"replaceAll" ontology-iri "^.*//" ""))
	   do
	      (unless (gethash ontology-iri seen)
		(format f "  <uri name=~s uri =~s/>~%" ontology-iri partial-path)
		(setf (gethash ontology-iri seen) t))
	      (unless (gethash imported-iri seen)
		(format f "  <uri name=~s uri =~s/>~%" imported-iri partial-path)
		(setf (gethash ontology-iri seen) t))
