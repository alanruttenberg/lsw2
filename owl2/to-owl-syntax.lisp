...
(defun to-owl-syntax (ont syntax &optional dest)
  ...
  (let ((ont (if (v3kb-p ont) (v3kb-ont ont) ont))
    ...
    (if (null dest) 
        ;; get rid of the comments and blank lines
        (cond ((member syntax '(:turtle :ntriples :functional))
               (#"replaceAll" (#"replaceAll" (#"toString" target) "(?m)^#.*$" "") "(?m)$\\s*$" ""))
              ((eq syntax :rdfxml)
               (#"replaceAll" (#"replaceAll" (#"toString" target) "(?s)<!--.*?-->" "") "(?m)$\\s+$" (string #\newline)))
              ((eq syntax :manchester)
               (#"replaceAll" (#"toString" target) "(?m)($\\s+$)+" (string #\newline)))
              (t (#"toString" target)))
	(if (eq t dest) 
	    (values)
	    (truename dest)))))
