...
(defrdfm builtin-atom
    (:pattern (builtin-atom ?builtin (?+ ?var)) :case :subscript-free :head (:blank ?x))
  (triple (:blank ?x) !rdf:type !swrl:BuiltinAtom)
  (triple (:blank ?x) !swrl:builtin ?builtin)
  (loop for v in ?var for i from 1 collect 
       (triple (:blank ?x) (swrl-argument-predicate i) v)
     )
  )
...
(defrdfm rule
  ...
  (t (expand-simple-rule-syntax ?ant ?cons))
  )

(defun swrl-argument-predicate (i)
  (make-uri nil (format nil "swrl:argument~a" i)))

(defun expand-simple-rule-syntax (ants cons)
  (flet ((expand-one (term)
	   (cond ((and (consp term)
		       (uri-p (car term))
		       (equal (length term) 2))
		  `(class-atom ,@term))
