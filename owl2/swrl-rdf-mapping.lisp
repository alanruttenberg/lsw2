...
(defun test-swrl ()
  ...
  (with-ontology foo () 
    ...
    (to-owl-syntax foo :rdfxml))
  (with-ontology foo () 
      ((asq (declaration (data-property !total))
...
(defun test-unit-conversion ()
  (with-ontology foo ()
      ((asq 
	(declaration (dataproperty !value-in-centimeters))
	(declaration (object-property !unit))
	(declaration (data-property !value-in-centimeters))
	(declaration (data-property !value))
	(declaration (named-individual !inches) )
	(declaration (named-individual !test))
	(data-property-assertion !value !test (:literal 1 !xsd:float))
	(data-property-assertion !unit !test !inches)
	(rule (!unit !?thing !inches) 
	      (data !value !?thing !?value)
	      (multiply !?centimeters !?value  (:literal 2.54 !xsd:float))
	      ->  (data !value-in-centimeters !?thing !?centimeters))))
    (princ (to-owl-syntax foo :functional ))
    (sparql '(:select (?p ?value) () (!test ?p ?value))
	    :kb foo :use-reasoner :sparqldl :trace t)
    foo))
