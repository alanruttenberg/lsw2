  ...
  (let ((use-special-unification (and invert (not (test-option17?)))))
    (declare-relation1
     ...
     :equal-code (and use-special-unification
                      (lambda (args1 args2 subst fn)
                        (declare (ignore fn))
                        (equal-jepd-relation-atom-args-p args1 args2 subst invert)))
     :variant-code (and use-special-unification
                        (lambda (cc args1 args2 subst matches fn)
                          (declare (ignore fn))
                          (variant-jepd-relation-atom-args cc args1 args2 subst matches invert)))
     :unify-code (and use-special-unification
                      (lambda (cc args1 args2 subst fn)
                        (declare (ignore fn))
                        (unify-jepd-relation-atom-args cc args1 args2 subst invert)))
     :index-type (and use-special-unification :jepd)
     :ordering-status (if use-special-unification :multiset :left-to-right)
...
(defun declare-equality-jepd-relation (relname sort n equality)
  (when equality
    ...
    (assert `(forall ((?x :sort ,(first sort)))
            ...
            :name (intern (format nil "~A-~A" relname :equality) :keyword)
            :supported nil)))

(defun declare-jepd-relation-intersection (relname rel sort invert)
  (let ((intersection (intern (format nil "~A-~A" relname :intersection) :snark)))
    (declare-relation1
     intersection 4
     :rewrite-code (list
                    (lambda (atom subst)
...
(defun declare-jepd-relations (relname sort composition invert equality names more-names)
  ...
  (let ((n (length names))
    ...
    (let ((table composition)
          (composition (intern (format nil "~A-~A" relname :composition) :snark)))
      (declare-relation1
       composition 5
       :rewrite-code (list
                      (lambda (atom subst)
  ...
  (let ((interval-sort (time-interval-sort-name?))
    ...
    (when (and intervals points)
      ...
      (let* ((relname '$$time-pi)
        ...
        (let ((composition (intern (format nil "~A-~A" relname :ii-composition) :snark)))
          (declare-relation1
           composition 5
        ...
        (let ((composition (intern (format nil "~A-~A" relname :pp-composition) :snark)))
          (declare-relation1
           composition 5
        ...
        (let ((composition (intern (format nil "~A-~A" relname :pi-composition1) :snark)))
          (declare-relation1
           composition 5
        ...
        (let ((composition (intern (format nil "~A-~A" relname :pi-composition2) :snark)))
          (declare-relation1
           composition 5
