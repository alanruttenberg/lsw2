...
(defun input-function-as-relation-result-sort1 (head args)
  (declare (ignore args))
  (let ((fsds (function-sort-declarations head)))
    (cond
     ((or (null fsds) (not (null (rest fsds))))
      (top-sort))
     (t
...
(defun input-function-as-relation-result-sort2 (head args)
  (let* ((arity (+ (length args) 1))
         ...
         (fsds (and (neq none rel) (function-sort-declarations rel))))
    (cond
     ((or (null fsds) (not (null (rest fsds))))
      (top-sort))
     (t
      (fsd-arg-sort (first fsds) arity)))))

(defun input-function-as-relation-result-sort (head args)
  (let ((resultsort (sort-intersection
        (input-function-as-relation-result-sort1 head args)
...
(defun input-function-as-relation (head args polarity &optional (new-head-name (function-name head)))
  ...
  (let* ((resultsort (input-function-as-relation-result-sort head args))
         (resultvar (if (top-sort? resultsort)
                        (make-symbol (format nil "~A~D" (first (variable-symbol-prefixes?)) (nonce)))
                        (make-symbol (format nil "~A~A~D" (first (variable-symbol-prefixes?)) resultsort (nonce)))))
...
(defun input-variables-in-form (expr vars substitution &optional result)
  ...
  (cond
    ...
    ((eq 'quote (first expr))
     result)
    ((let ((v (find-symbol-table-entry (first expr) :logical-symbol)))
	...
	(let* (conc-name
	       ...
	       (x (cond
                   ...
                   ((and (not (getf (rest var-spec) :sort-unknown))
                    ...
                    (newsym (format nil "~A-~A" sort :skolem)))
                   (t
                    (newsym :skolem)))))
  ...
  (let ((n (gethash conc-name *new-symbol-table* 0)))
    (cond
     ...
     (t
      ...
      (intern (format nil "~A~D" conc-name n) :snark-user)))))  

...
(defun input-term (term &optional (*input-wff-substitution* nil))
