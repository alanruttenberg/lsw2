...
(defun declare-feature (name &key parent children-incompatible iff implies)
          ...
          (let ((v (the-feature iff 'error)))
            ...
            (cond
             ...
             (t
              (push name (feature-users-in-name-table v))))
            (when cf
              (setf (feature-name cf) (make-symbol (concatenate 'string "*" (symbol-name name) "*")))
   ...
   ((feature-combo? x)
    (or (feature-combo-name x)
        (let ((expr (unthe-feature x)))
          (if (atom expr) expr (setf (feature-combo-name x) (make-symbol (format nil "~A~{&~A~}" (second expr) (rest (rest expr)))))))))
   (t
    nil)))
...
(defun print-feature (n)
  (princ (or (feature-name n) (feature-preorder-min n)))
  n)
