...
(defun size (x &optional subst)
  (dereference
   ...
   :if-constant 1
   :if-variable 1
   :if-compound-cons (+ (size (carc x) subst) (size (cdrc x) subst) 1)
   :if-compound-appl (let ((head (heada x)))
                       (cond
                        ((function-associative head)	;size of f(a,b,c) = size of f(a,f(b,c)) = 5
                         (let ((args (argsa x)))
                           (+ (loop for x1 in args sum (size x1 subst))
                              (+ 1 (length (rest (rest args)))))))
                        (t
                         (+ (loop for x1 in (argsa x) sum (size x1 subst))
                            1))))))

(defun weight (x &optional subst)
  (dereference
   x subst
   :if-constant (constant-weight x)
   :if-variable (variable-weight?)
   :if-compound-cons (+ (weight (carc x) subst) (weight (cdrc x) subst) (function-weight *cons*))
   :if-compound-appl (let ((head (heada x)))
                       (dolist (fun (function-weight-code head)
                                    (cond
                                     ((function-associative head)
                                      (let ((args (argsa x)))
		                        (+ (loop for x1 in args sum (weight x1 subst))
			                   (* (function-weight head) (+ 1 (length (rest (rest args))))))))
                                     (t
                                      (+ (loop for x1 in (argsa x) sum (weight x1 subst))
