...
(defun coder (axioms target &rest options
              ...
              kill all-proofs must-use resume hints reverse-cd
    ...
    (prog->
      ...
      (loop for nsteps from min to max
            do (let (targets-found)
                 ...
                 (force-output)
	         (setf targets-found (coder1 axioms target nsteps cd op kill all-proofs must-use resume hints steps-to-use steps-to-use-count))
	         (setf resume nil)
                 (format t "~%~D steps in ~D seconds"
                         *coder-step-count*
                         (round (- (get-internal-run-time) *coder-start-time*) internal-time-units-per-second))
                 (when targets-found
...
(defun coder1 (axioms target nsteps cd op kill all-proofs must-use resume hints steps-to-use steps-to-use-count)
