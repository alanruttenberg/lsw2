  ...
  (unless (null element) 
    (let ((name (car element))
      ...
      (if (cond ((consp name) 
          ...
          (if (and more-tags children (listp (car children)))
              (some (lambda(e) (apply 'find-element-with-tag e more-tags)) children)
              element)
          (if (listp (car children))
              (some (lambda(e) (apply 'find-element-with-tag e tag more-tags)) children))
          ))))
  ...
  (unless (null element)
    (let ((name (car element))
      ...
      (if (cond ((consp name) 
          ...
          ;; we've matched
          (if (and more-tags children (listp (car children)))
              (loop for child in children for found = (apply 'find-elements-with-tag child more-tags) when found append found)
              (list element))
          (if (listp (car children))
              (loop for child in children for found = (apply 'find-elements-with-tag child tag more-tags) when found append found)
              )))))
