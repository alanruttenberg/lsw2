...
(defmethod read-obo-key-values ((g obo) stream)
  (loop for line = (read-line stream nil :eof)
     for (tag value) = (unless (eq line :eof) (car (all-matches line "^(\\S+):\\s*(.*?)\\s*(![^\"]+?){0,1}$" 1 2)))
     until (or (null tag) (eq line :eof))
	 ...
	 (destructuring-bind (synonym sources type label) (car (all-matches value "\"(.*?)\"\\s+((EXACT|RELATED|BROAD|NARROW)\\s+){0,1}(([^\\[]+?)\\s+){0,1}\\[(.*)\\]" 1 6 3 5))
