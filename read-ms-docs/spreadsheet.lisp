...
(defun each-row-in-sheet  (file fn &key (sheet :first))
  ...
  (let* ((sheets (unless (java-object-p sheet) (list-sheets  :file file)))
    ...
    (jss:with-constant-signature ((getrow "getRow") (getcell "getCell") (get-number-of-cells "getLastCellNum"))
	   ...
	   (let ((value
		   (cond ((eql type (load-time-value (get-java-field 'org.apache.poi.hssf.usermodel.HSSFCell "CELL_TYPE_STRING")))
			 ...
			 ((eql type (load-time-value (get-java-field 'org.apache.poi.hssf.usermodel.HSSFCell "CELL_TYPE_NUMERIC")))
			  (if (#"isCellDateFormatted" 'HSSFDateUtil cell)
			      (getdate cell)
			      (getnumber cell)))
			 (t (error "unknown cell type ~a ~a" cell type)))))
	     (values value formula (getstyle cell)))))))

(defun excel-serial-number-to-date (string)
  (let* ((jdate (#"getJavaDate" 'org.apache.poi.ss.usermodel.DateUtil (coerce (parse-integer string) 'double-float)))
	 (year (#"getYear" jdate))
	 (date (#"getDate" jdate))
	 (month (#"getMonth" jdate)))
    (format nil "~4,'0d-~2,'0d-~2,'0d" (+ year 1900) month date)))
